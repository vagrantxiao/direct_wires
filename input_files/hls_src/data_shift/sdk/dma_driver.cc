...
int dma_inst::WR2TxBuffer()
{
	...
	for(i = 0; i < num_3d_triangles; i ++) {
		TxPacket[i*16+0] = triangle_3ds[i][0];
		TxPacket[i*16+1] = triangle_3ds[i][1];
		TxPacket[i*16+2] = triangle_3ds[i][2];
		TxPacket[i*16+3] = triangle_3ds[i][3];
		TxPacket[i*16+4] = triangle_3ds[i][4];
		TxPacket[i*16+5] = triangle_3ds[i][5];
		TxPacket[i*16+6] = triangle_3ds[i][6];
		TxPacket[i*16+7] = triangle_3ds[i][7];
		TxPacket[i*16+8] = triangle_3ds[i][8];
	}
...
int dma_inst::CheckData(void)
{
	...
	for (int i = 0, j = 0, n = 0; n < 16384; n ++ )
	{
	  ...
	  frame_buffer_print[i][j++] = u8 (temp&0x000000ff);
	  frame_buffer_print[i][j++] = u8 ((temp&0x0000ff00)>>8);
	  frame_buffer_print[i][j++] = u8 ((temp&0x00ff0000)>>16);
	  frame_buffer_print[i][j++] = u8 ((temp&0xff000000)>>24);
	  if(j == 256)
	  {
		i++;
		j = 0;
	...
	for (int j = 256 - 1; j >= 0; j -- )
	{
	  ...
	  {
	    int pix;
	    pix = (int) frame_buffer_print[i][j];
        if (pix)
          printf("1");
        else
          printf("0");
