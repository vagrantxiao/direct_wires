...
int main1(int argc, char ** argv)
{
	...
	printf("Hello world\n");
	 ...
	 int label_exp = 9;
	 knn_set[K_CONST*4*label_exp] = 9;
	 knn_set[K_CONST*4*label_exp+1] = 10;

	label = knn_vote(knn_set);
	printf("label = %d\n", label);
}
...
int main(int argc, char ** argv)
{
  ...
  // sdsoc version host code
    ...
    hls::stream<ap_uint<128> > Input_1("main_input");
    hls::stream<ap_uint<128> > Output_1("main_output");
    ...
    //Input_1.write(0);
    for (int i = 0; i < NUM_TRAINING / 2; i ++){
    	Input_1.write(training_in0[i].range(255, 128));
    	Input_1.write(training_in0[i].range(127, 0));
    }
    /*for (int i = 0; i < NUM_TRAINING / 2; i ++){
    	Input_1.write(training_in0[i]);
    }*/
    // first call: transfer data
    //DigitRec(training_in0, test_in, result, 0);
//    DigitRec(Input_1, Output_1);
//    Input_1.write(1);
    for (int i = 0; i < NUM_TRAINING / 2; i ++){
		Input_1.write(training_in1[i].range(255, 128));
		Input_1.write(training_in1[i].range(127, 0));
	}
    /*for (int i = 0; i < NUM_TRAINING / 2; i ++){
    	Input_1.write(training_in1[i]);
    }*/
    for (int i = 0; i < NUM_TEST; i ++){
		Input_1.write(test_in[i].range(255, 128));
		Input_1.write(test_in[i].range(127, 0));
	}
    /*for (int i = 0; i < NUM_TEST; i ++){
    	Input_1.write(test_in[i]);
    }*/
  ...
  // sw version host code
    ...
    for(int i = 0; i < NUM_TEST; i ++)
    {
    	bit128 Output_tmp = Output_1.read();
