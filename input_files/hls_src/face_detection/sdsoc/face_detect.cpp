...
#pragma HLS INTERFACE ap_hs port=Output_1
  ...
  bit128 in_tmp1 = Input_1.read();
  bit128 in_tmp5 = Input_5.read();
...
#pragma HLS PIPELINE II=1
		...
		bit128 in_tmp2 = Input_2.read();
		bit128 in_tmp3 = Input_3.read();
		bit128 in_tmp4 = Input_4.read();
		in_tmp5 = Input_5.read();
		for(j=0; j<10; j++)
		{
#pragma HLS unroll
			sum_tmp[0][i*10+j] = (ap_int<12>)in_tmp1(j*12+11,j*12);
			sum_tmp[1][i*10+j] = (ap_int<12>)in_tmp2(j*12+11,j*12);
			sum_tmp[2][i*10+j] = (ap_int<12>)in_tmp3(j*12+11,j*12);
			sum_tmp[3][i*10+j] = (ap_int<12>)in_tmp4(j*12+11,j*12);
			sum_tmp[4][i*10+j] = (ap_int<12>)in_tmp5(j*12+11,j*12);
		}
	}
...
#pragma HLS PIPELINE
				...
				//#pragma HLS PIPELINE
					//data_req_0 = Input_2.read();
					//Output_1.write(data_req_0(i*32+63, i*32+32));
				//}
        		haar_counter++;
        	 }
...
#pragma HLS PIPELINE
			in_tmp = Input_1.read();
			INPUT_1: for(i = 0; i<5; i++){
				II_0[i*25+j] = in_tmp(18*i+17,18*i);
			}
		}
	}
...
#pragma HLS unroll
//#pragma HLS PIPELINE
		data_req_0 = in_tmp(32*i+31,32*i);
...
#pragma HLS unroll
//#pragma HLS PIPELINE
		data_req_0 = in_tmp(32*i+31,i*32);
		coord2: for(j=0; j<3; j++){
		#pragma HLS unroll
			II_x[j][i] = data_req_0(j*10+4,j*10);
			II_y[j][i] = data_req_0(j*10+9,j*10+5);
...
#pragma HLS INTERFACE ap_hs port=Output_5
  ...
  /* The rectangle co-ordinagte values for all the classifiers */
  #include "haar_dataRcc_with_partitioning.h"
//#pragma HLS ARRAY_PARTITION variable=coord_loc cyclic factor=4 dim=1
...
#pragma HLS INTERFACE ap_hs port=Output_1
	...
	static unsigned char Data[IMAGE_HEIGHT/2][IMAGE_WIDTH];
	  ...
	  LOAD_i: for( i = 0; i < IMAGE_HEIGHT/2; i++){
	    LOAD_j: for( j = 0; j < IMAGE_WIDTH/16; j++){
	    	bit128 Input_tmp = Input_1.read();
	      LOAD_k: for( k = 0; k < 16; k++){
#pragma HLS pipeline II=1
	    	  Data[i][j*16+k] = Input_tmp(8*k+7, 8*k);
	      }
	  ...
	  CROSS_i: for( i = IMAGE_HEIGHT/2; i < IMAGE_HEIGHT; i++){
	    CROSS_j: for( j = 0; j < IMAGE_WIDTH/16; j++){
	    	bit128 Input_tmp = Input_1.read();
	    	CROSS_k: for( k = 0; k < 16; k++){
#pragma HLS pipeline
...
#pragma HLS ARRAY_PARTITION variable=Data cyclic factor=16 dim=0
	...
	{
		GEN_2: for( j = 0; j < IMAGE_WIDTH/16; j++)
		{
#pragma HLS PIPELINE II=1
			bit128 Input_tmp;
			GEN_3: for (k=0; k<16; k++){
#pragma HLS UNROLL
				Input_tmp(k*8+7, k*8) = Data[i][j*16+k];
			}
