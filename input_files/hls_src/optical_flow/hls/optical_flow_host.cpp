...
int main(int argc, char ** argv) 
{
  ...
  // opencl version host code
    ...
    for (int i = 0; i < MAX_HEIGHT; i++) 
    {
      ...
      {
        tmpframes(7 ,  0) = imgs[0].Pixel(j, i, 0);
        tmpframes(15,  8) = imgs[1].Pixel(j, i, 0);
        tmpframes(23, 16) = imgs[2].Pixel(j, i, 0);
        tmpframes(31, 24) = imgs[3].Pixel(j, i, 0);
        tmpframes(39, 32) = imgs[4].Pixel(j, i, 0);
        tmpframes(63, 40) = 0;  
      }
    }
  ...
  // sdsoc version host code
   ...
   // }
    ...
    //static hls::stream<stdio_t> frames("test1");
    static hls::stream< ap_uint<128> > frames("test1");
    static hls::stream< ap_uint<128> > flo_out("test2");
     // pack the values
    /*for (int i = 0; i < MAX_HEIGHT; i++)
    {
      for (int j = 0; j < MAX_WIDTH; j++) 
    ...
    optical_flow_mono(frames, flo_out);
    int num = flo_out.read();
    printf("We should receive %d values\n", num);
    for (int i = 0; i < MAX_HEIGHT; i++) 
    {
      for (int j = 0; j < MAX_WIDTH/2; j++)
      {
    	  tmpframes = flo_out.read();
    	  outputs[i][j*2].x(31,0) = tmpframes(31,0);
    	  outputs[i][j*2].y(31,0) = tmpframes(63,32);
    	  outputs[i][j*2+1].x(31,0) = tmpframes(95,64);
    	  outputs[i][j*2+1].y(31,0) = tmpframes(127,96);
      }
    }
  ...
  printf("The right Average error should be 32.059417\n");
