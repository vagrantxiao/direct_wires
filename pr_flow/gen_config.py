...
class packets_gen:
	def __init__(self, packet_bits, addr_bits, port_bits, bram_addr_bits, 
							payload_bits, bram_addr, freespace, input_port_offset, output_port_offset):
  	# Initialize the specs
	...
	def generate_packets_of_loc(self, conn_parser, func_loc_dict):
		"""This function make up all the dicts and lists for the packets sent and received 
		by ARM"""
...
class c_source_gen:
	...
	def generate(self, 
								init_pkt_arm2leaf_num_list, 
								ack_pkt_arm2leaf_num_dict, 
								ack_pkt_leaf2arm_num_dict, 
								data_pkt_arm2leaf_num_dict,
                                                                board):
		...
		for src_function in ack_pkt_arm2leaf_num_dict.keys():	
			for dst_function in ack_pkt_leaf2arm_num_dict.keys():
				self.generate_stream_operation_for_each_leaf(
								src_function,
								dst_function,
								config_c_file,
								ack_pkt_arm2leaf_num_dict[src_function], 
								ack_pkt_leaf2arm_num_dict[dst_function], 
								data_pkt_arm2leaf_num_dict[dst_function])
	...
	def generate_common_func(self, config_c_file, init_pkt_arm2leaf_num_list, board):
		...
		for packet_value in init_pkt_arm2leaf_num_list:
			...
			config_c_file.write("	write_to_fifo(" + str(hex(packet_value_1)).replace('L', '') + ', '
			                                               + str(hex(packet_value_0)).replace('L', '') + ", &ctrl_reg);\n")
		config_c_file.write("\n")
		config_c_file.write("\n")
		config_c_file.write("}\n")
		
	def generate_stream_operation_for_each_leaf(self,
								src_function,
								dst_function,
								config_c_file,
								ack_pkt_arm2leaf_num, 
								ack_pkt_leaf2arm_num, 
								data_pkt_arm2leaf_num):	
		
		config_c_file.write("int stream_" + src_function +
												"_ARM_" + dst_function + "(int op_type, int i){\n")
		...
		config_c_file.write("                   write_to_fifo(" + str(hex(value_var_1)).replace('L', '') + '+addr, '
		                                               + str(hex(value_var_0)).replace('L', '') + "+i);\n")
...
class auto_config:
  ...
  def generate(self):
    ...
    output_source_file_name = self.prflow_params['workspace']+'/F006_sdk/config_'+self.prflow_params['benchmark_name']+'.cc'
    output_header_file_name = self.prflow_params['workspace']+'/F006_sdk/config_'+self.prflow_params['benchmark_name']+'.h'
    ...
    packets_gen_inst = packets_gen(packet_bits,
                                           addr_bits,
                                           port_bits,
                                           bram_addr_bits,
                                           payload_bits,
